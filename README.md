...
公共模板同样会在 `network-changed` 事件和自定义周期中检查 IP，并提供手动检查面板。Surge 会先通过 `64.ipcheck.ing/geo` 获取真实出口 IP 和地区信息，仅在 IP 变化时提交给 Gatekeeper；服务端会将出口 IPv4 转为 `/24`。

> 注意：此模式会减少 API 调用，但如果管理员在后台清空了该设备的网段，而设备出口 IP 一直未变，自动任务不会重复提交。此时可点击 Surge 面板右上角刷新按钮手动强制上报。
