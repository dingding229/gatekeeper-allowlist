...
- 参考 po0fw 的提交逻辑，每次有效触发都调用 Gatekeeper API，由服务端幂等处理已存在的网段，不重复占用槽位也不推进 FIFO。
...
公共模板同样会在 `network-changed` 事件和自定义周期中上报，并提供手动刷新面板。Surge 会先通过 `64.ipcheck.ing/geo` 获取真实出口 IP 和地区信息，再提交给 Gatekeeper；查询失败时仍会提交，由服务端使用请求来源 IP。服务端会将出口 IPv4 转为 `/24`，同一网段重复上报不会占用新槽位。
