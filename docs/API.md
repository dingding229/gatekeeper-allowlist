...
省略 `ip` 时使用请求来源 IP；也可以显式传入：

```json
{"ip":"203.0.113.8","source":"office"}
```

新 IP 返回 `201`，已存在的 IP 返回 `200`。每个 Key 最多保留 3 个不同 IP，第 4 个会淘汰最早加入的地址。
...
管理员接口供自带 Web 后台使用，通过 HttpOnly 会话 Cookie 鉴权，不作为公开集成接口。防火墙快照接口只应允许本机访问。
